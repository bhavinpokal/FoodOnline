...
class Vendor(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.pk is not None:
            status = Vendor.objects.get(pk=self.pk)
            if status.is_approved != self.is_approved:
...
HOURS_OF_DAY = [(time(h, m).strftime('%I:%M %p'), time(h, m).strftime(
    '%I:%M %p')) for h in range(0, 24) for m in (0, 30)]
